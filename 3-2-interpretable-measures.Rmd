...
<!-- We also have to search for significant measures since creating model with thousands of features do not make it interpretable. We done it training models -- rpart or ranger -- with additional 1000 features and using DALEX variable importance we choose around 10 most important features. We done it twice so at the and we had around 10 the most important features.  -->
...
<!-- It was done on 5 cross validation and when the model was trained on 4 folds with new observations made by smote it was tested on the last fold without any new observations. -->
...
| Number | Operation  | logreg | kknn | rpart | improve |
|----|-------|---|---|---|--|
| 0a | Base | 0.7347 | 0.7275 | 0.5000 | no |
| 0b | Rpart tuning | 0.7347 | 0.7275 | 0.7369 | yes |
| 1a | Normalization | 0.7347 | 0.7272 | 0.7369 | no |
| 1b | Outlier reduction | 0.7433 | 0.7315 | 0.7394 | yes |
| 1c | Logarithm | 0.7435 | 0.7175 | 0.7254 | no? |
| 2a | Outlier reduction and normalization | 0.7433 | 0.7320 | 0.7394 | yes |
| 2b | Logarithm and outlier reduction | 0.7435 | 0.7168833 | 0.7246 | no |
| 3a | Gain-ratio discretization | 0.7433 | 0.7317 | 0.7394 | no |
| 3b | RSafe | 0.7437 | 0.7176 | 0.7338 | no |
| 4a | New features selected by ranger | 0.7472 | 0.7288 | 0.7334 | yes |
| 4b | New features selected by rpart | 0.7453 | 0.7311 | 0.7392 | yes |
| 4c | Halstead measures | 0.7448 | 0.7311 | 0.7376 | no |
...
Table: AUC in white-box models

Since at the begining rpart has the 0.5 AUC first we tuned the rpart hiperparameters and there was significant improve in result. 

Then as step 1a-1c we tried 3 basic transformations of data as normalization of all columns, outlier reduction and logarithm of nearly all columns (without one hot encoded source). As we can see the best improve gave otliers reduction so in step 2a we tried to add normalization to that and in step 2b we tried to reduce outliers after applying logarithm. Only outliers reduction and normalization was good enough to use it in following experiments.

Next we use discretization of some columns. Since all columns were continous numbers it could be potencially usable. Unfortunately, any of gain-ratio method from funModeling (3a) and RSafe (3b) make statistically important better result. Using partial dependency plot on the black-box model do not show that there is variable which can be discretized.
<!-- referencja na wykres pdp + wykres, czy nie warto? -->

Then we tried to create new measures based on basic Halstead measures. We selected best measures by DALEX variable importance in one of two learners: ranger or rpart. Then on the best 10 measures + basic measures we run the models but the results in the table shows only a little better performance of logistic regression and knn. The most promising result was the improvement of logistic regression in 4a.

Since the classes are unbalanced - there are 20\% of defects so we tried the smote oversampling of defects class. There is a huge improvement especially in decision tree and the best result is in dataset with only outliers reduction and normalization without any new features. So this is our final model and this model beats ranger AUC.

![](3-2-graphs/rpart_result.png)

On the plot there is the final rpart model with proportions of each improvement. Zero from scale means the base rpart model at the beging and 100 is equivalent of black-box model -- ranger. The most important was hiperparameters tuning, then oversampling with smote and the rest give only a little improvement.
