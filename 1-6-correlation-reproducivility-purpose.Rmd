...
licznik <- chnk %>% select(-Author, -length, -reproducibility.scale) %>% summarise_all(sum) %>% melt
tot_length <- chnk %>% select(-Author, -reproducibility.scale) %>% melt(id.vars = "length") %>%
...
![text](./1-6-files/korelacje_celu_chunki.png)
\
